...
- [ ] 24-hour burn-in test: Pi + real detector running unattended indoors, no thermal shutdown (review via ssh tail of system_metrics JSONL)
...
**2026-05-08 — CPU temp warning tier, system metrics logger, iOS uptime (Part 2)**
Pi: (1) `notifier.py` — added `cpu_temp_warning` tier at >65°C (priority `default`, tag `thermometer`); existing `cpu_temp_high` bumped to priority `high` and body updated ("Throttling imminent."). `evaluate_and_notify()` now returns `list[str]` of fired condition keys. `test_ntfy.sh` gains `temp_warning` condition (cpu_temp_c=70) and adds it to `all`. (2) `system_logger.py` (new) — `SystemLogger` class with 60s metrics thread writing to `data/system_metrics_YYYY-MM-DD.jsonl`, daily UTC rotation, append-only JSONL. Metrics include: cpu_temp_c, cpu_pct, ram_pct/mb, ssd_free_gb/used_pct, detector_fps, yolo_running, cam_reachable, hdmi_reachable, uptime_s. Uses psutil (7.2.2 already in venv) + existing storage_monitor/detector/cam_client refs (no new polling). `log_event(category, details)` for service events: service_start, recording start/stop, ntfy_sent, passive agent errors. (3) `api_server.py` — imports and instantiates SystemLogger after detector ready; logs service_start event at startup; hooks recording start/stop; logs ntfy_sent when evaluate_and_notify fires; adds `uptime_s` and `system_log_path` to `/status`. iOS: (4) `DataViewModel.swift` — `uptimeSeconds: Int?` published var, decoded from `uptime_s` in HealthPoll. (5) `SettingsTabView.swift` — new ENCLOSURE card (below DIAGNOSTIC): UPTIME row formatted as Xh Ym, LOG row showing today's metrics file path. Monitoring reference: ssh pi@raspberrypi.local "tail -50 /home/pi/hhrcs/data/system_metrics_$(date -u +%Y-%m-%d).jsonl | python3 -m json.tool". Temp tiers: <65°C normal · 65–80°C warning ntfy · >80°C critical ntfy.
...
**2026-05-09 — Page titles, codec fix, dash consistency, WARM UI default, splash screen (iOS only)**
iOS-only. Section 1 (page titles): `DataTabView` and `SettingsTabView` bodies wrapped in `VStack(spacing: 0)` — title bar (`HStack { Text("DATA"/"SETTINGS") ... }.frame(height: 32)`) + `HRule()` above existing ScrollView; font: `Theme.label(size: 11)`, `Theme.labelTracking`, `Theme.text1`, `.semibold` — matches FIELD tab strip and BMPCC stillDataBar chrome. `.background(Theme.background)` and `.clipShape`/`.onAppear` moved to outer VStack. Section 2 (codec truncation): `DataTabView.cameraSection` `codecDisplay` changed from `"\(base) · \(v)"` to variant-only (e.g. "Proxy") when variant exists, base codec otherwise — "ProRes · Proxy" no longer overflows the 1/3-width cell. Section 3 (dash consistency): `DataTabView.enclosureSection` — 4 `"--"` placeholders changed to `"—"` (em dash) matching camera section. Section 4 (WARM UI): `AppSettings.swift` — default changed `false` → `true`; `HHRCSApp.swift` — `applyWarmUIDefault()` migration called in `init()`: writes `ui.warmAccent=true` + sentinel `ui.warmAccent.defaultSet=true` to UserDefaults if sentinel absent (existing installs get warm default on next launch). `Theme.swift` `NeutralToggleStyle` — gains `activeColor: Color = Theme.text1` param; track fill changed from `Color.white.opacity(0.85)` to `activeColor.opacity(0.85)`. `SettingsTabView.ToggleRow` — observes `AppSettings.shared`, passes `settings.activeColor` to `NeutralToggleStyle`. `SettingsTabView.systemCard` — `ToggleRow` replaced with inline `HStack`: 6pt `Circle().fill(dawnDuskWindows ? settings.activeColor : Theme.tertiary)` dot + label + toggle with `NeutralToggleStyle(activeColor:)`. Section 5 (splash screen): `Assets.xcassets/LaunchBackground.imageset` created with `LaunchBackground@3x.png` (1290×2796, 3× slot only); `LaunchView.swift` body — `Theme.background.ignoresSafeArea()` replaced by `Image("LaunchBackground").resizable().scaledToFill().ignoresSafeArea().overlay(Color.black.opacity(0.45))`; all existing content unchanged below overlay.

**2026-05-09 — Scrollbars, tab strip, header alignment, auto-record bug, notification split (iOS + Pi)**
iOS + Pi. Section 1 (scrollbars): `.scrollIndicators(.hidden)` applied globally — 9 ScrollView instances across `SettingsTabView` (2), `DataTabView` (2), `StillsGalleryView`, `NotesTabView`, `SessionLogView`, `ControlTabView`, `InstrumentComponents`; horizontal scroll in DataTabView updated from `showsIndicators: false` param to `.scrollIndicators(.hidden)` modifier. Section 2 (FIELD tab strip): `NotesTabView` tab strip — `VStack(spacing: 5)` wrapper + `Rectangle().fill(...)` underline removed from each tab button; active state updated to `.semibold` weight + `Theme.text1`, inactive `.regular` + `Theme.tertiary`. Section 3 (header alignment): FIELD tab strip `HStack` changed from `.padding(.top, 16).padding(.bottom, 10)` to `.frame(height: 32)` — matches BMPCC `stillDataBar` frame height. Section 4 (auto-record bug): `state_machine.py` `open_window()` — removed `if self.state == RecordState.IDLE: self._start_recording(trigger)` block; window open now only sets `window_open = True`; recording starts only via `detection_event()` (when window open + animal detected) or `force_start()` (manual). Root cause: startup called `open_window(SCHEDULED)` before `detector.start()`, immediately firing `on_record_start` → `camera.record_start()`. Section 5 (duplicate pollers): `DataViewModel` — added `task?.cancel()` guard before each `Task { }` assignment in `startHealthPolling()`, `startLogPolling()`, `startNotificationPolling()` — prevents duplicate pollers when `scenePhase` fires `.active` multiple times. Notifications split: `AppSettings` — `notifyRecording` replaced by `notifyRecordings` + `notifyStills` (separate `@Published` vars + `Keys`); `NotificationPoller` — `"recording.started"/"recording.stopped"` map to `notifyRecordings`, `"still.captured"` maps to `notifyStills`; `SettingsTabView` notifications card — single "RECORDING & STILLS" row split into "RECORDINGS" + "STILLS" rows. HDMI still spam: `api_server.py` `/hdmi/still` and `/still/trigger` endpoints — removed `notifications.emit("still.captured", ...)` and `log_agent_entry("observation", ...)` calls (periodic StillPoller triggers every 5s were generating a notification per call). Pi deployed (api_server.py, state_machine.py).

**2026-05-09 — UI cleanup, diagnostic dot fixes, Pi CONTEXT.md injection (iOS + Pi)**
iOS + Pi. Section 1 (FEED/CameraTabView): record button removed from CAM page — `OperatorControlRow` gains `showRecord: Bool = true`; `.live` case sets `showRecord: false`; layout reflows to indicator + capture button only. Section 2 (FIELD): `StillsGalleryView` — ConfirmationCard lifted from `StillCell` inline overlay to `StillsGalleryView`-level ZStack; `pendingDelete: CapturedStill?` state drives full-screen scrim + centered card; `StillCell` gains `onRequestDelete` callback. `agent.py` — `_load_context_md()` reads `/home/pi/hhrcs/CONTEXT.md` at import and on mtime change; content prepended to Tier 2 Claude API system prompt on every call; `_context_md_text`/`_context_md_mtime` module-level state. Section 3 (DATA/DataTabView): camera card rebuilt — removed RESOLUTION, FRAME RATE, RECORDING, LENS, CODEC VAR separate cells; CODEC+VARIANT combined into single cell (e.g. "ProRes · Proxy"); new row order: CODEC·SHUTTER, ISO·WB·GAIN, MEDIA SLOT·REC TIME LEFT, FORMAT, MEDIA VOL·CLIP COUNT·SPACE REM. Section 4 (SETTINGS/SettingsTabView): card order rewritten to DIAGNOSTIC → ENCLOSURE → DEPLOYMENT → REFRESH INTERVAL → DETECTOR THRESHOLD → ACCESS → SYSTEM → AGENT → NOTIFICATIONS → DEPLOYMENT CHECKLIST → DISPLAY; `piSdStatus` fixed — now reads `vm.piSdUsedPct` (microSD) with >80% yellow / >95% red thresholds; `selectLocation` changed to call `locationSearch.setDisplayAddress(coords.address)` instead of `clearQuery()` so selected address populates TextField. `LocationSearchViewModel` gains `setDisplayAddress(_:)` — sets query, cancels debounce, clears results (no new search fires). Section 5 (DiagnosticRecovery + DataViewModel + Pi): `DiagnosticRecovery.swift` — comment block added documenting all 8 dot→field→threshold mappings; `PiSdDetailView` rewritten to show Pi microSD data (DEVICE, USED%); `CamCfDetailView` gates REMAINING row behind `isActive` check (was showing SD remaining time when CF slot empty); `CamSdDetailView` same gating added. `DataViewModel.swift` — `piSdUsedPct: Double?` added with CodingKey `pi_sd_used_pct`. `api_server.py` — `_pi_sd_used_pct()` added (reads `df "/"` = microSD boot disk); field added to `/status`; stale `cam_battery` key removed from status dict (was KeyError after battery removal in prior session). `camera_http.py` — `get_full_state()` active media: `raw = media.get("deviceName") or None` (removed slot name fallback so empty deviceName → `cam_active_media_slot=None`). Pi deployed; verified `pi_sd_used_pct: 35.6`.

**2026-05-09 — Overnight burn-in diagnostic (read-only)**
Pulled `system_metrics_2026-05-09.jsonl` (12.44h, 736 samples). Key findings: 3 restarts in first 3h (01:30–02:55 UTC), longest uninterrupted window 9.53h with zero connectivity dropouts. Thermal: 43.5°C mean / 46.3°C max (well inside 60°C limit, +0.03°C drift). CPU: 97.0% mean (structural — 3-thread inference); memory flat at 11.8% mean with −0.004%/h slope (no leak). Detector fps: 1.786 mean, +0.009 drift in stable window — prior May 8 "drift signal" (+0.034) not reproduced; confirmed warm-up artifact. ssd_used_pct flat at 39.90% throughout (no recording writes). May 8 comparison: fps 1.653 mean / cpu 93.4% / temp 41.5°C. System assessed ready for overnight burn-in pending restart-cause investigation (journalctl recommended before starting run).

**2026-05-08 — Monochrome design system + WARM UI toggle (iOS only)**
Pure design pass across all iOS consumer files. No logic, networking, or storage changes. (1) `Theme.swift` rewritten: `accent`/`accentColor` removed entirely; new token set: `text1` (white 0.96), `text2` (white 0.62), `text3` (white 0.34), `text4` (white 0.14), `surface` (#0E0E0E), `rule` (white 0.06), `ok` (green diagnostic), `danger` (#FF3B30 destructive), `warmAccent` (warm amber). Migration aliases (`secondary`, `tertiary`, `cardBackground`, `text`, `cardLabel`) kept as forwarding computed vars for compile compat. Three-size font scale: `label(11)`, `body(14)`, `display(17)` with legacy aliases (`dataValue`, `dataLabel`, `sectionHeader`, `bodyMono`, `statusCaption`) forwarding to new scale. New dimension tokens: `rowVPadding=14`, `cardCorner=6`, `cardInset=16`. `NeutralToggleStyle` added — monochrome 38×22 pill toggle. (2) `AppSettings.swift`: added `import SwiftUI`, `@AppStorage("ui.warmAccent") var warmUI: Bool = false`, `var activeColor: Color` (returns `warmAccent` when warmUI on, else `text1`). (3) Consumer files updated: all `Theme.accentColor`/`Theme.accent` references replaced across `ContentView`, `ConfirmationCard`, `SettingsTabView`, `CameraTabView`, `ControlTabView`, `StillsGalleryView`, `DataTabView`, `InstrumentComponents`, `DetectionOverlayView`, `DiagnosticRecovery`, `SessionLogView`, `NotesTabView`, `AstroArcView`, `StillFrameView`, `LaunchView`, `DataViewModel`. Active-state / selected-state uses `settings.activeColor`; TextField cursor tint uses `Theme.text1`; ToggleRows use `NeutralToggleStyle`; reachable/healthy diagnostic states use `Theme.ok`; action buttons in non-interactive structs use `Theme.text1`; FINALIZING states use `Theme.text2`. (4) `SettingsTabView`: DISPLAY card added at top with WARM UI `ToggleRow`. (5) `CameraTabView`: page indicator active state — stroke overlay removed; active uses weight+`settings.activeColor` only. (6) `DataTabView`: `MetricCell` valueSize promoted to 17 for weather current temp/wind, enclosure temp/humidity, CPU temp; `BarCell` gains `valueSize` param (default 17). (7) `InstrumentComponents`: `BarCell` warn tier → `text2.opacity(0.8)`, crit tier → `danger`; subtitle font → `Theme.body(size:11)`. (8) `Assets.xcassets/AccentColor.colorset` deleted. No `UIAccentColorName` in Info.plist.

**2026-05-08 — BMPCC REST API field fixes (camera_http.py + iOS)**
Five targeted fixes confirmed against live BMPCC REST API: (1) Shutter angle path corrected — `/video/shutterAngle` → `/video/shutter`; response shape `{"shutterAngle": 18000}`; existing ÷100 parsing unchanged. (2) Battery removed entirely — `/system/batteryLevel` and `/system/power` both return 404; no working path exists in BMPCC REST API; `get_battery()` method deleted, `cam_battery` key removed from `get_full_state()` result, `camBattery` removed from iOS `DataViewModel` and `DataTabView` camera card. (3) Lens info fixed — `get_lens_info()` now returns `None` when `apertureStop` is 0 or None (manual lenses / no lens return 0.0 via `/lens/iris`, previously displayed as "f/0.0"). (4) Active media endpoint corrected — `get_active_media()` changed from `/media/active` (incomplete) to `/media/workingset`; finds entry where `activeDisk: true`; now populates `cam_remaining_record_time`, and three new fields: `cam_media_volume` (e.g. "A002"), `cam_media_clip_count` (int), `cam_media_space_remaining_gb` (remainingSpace ÷ 1e9, 1dp); iOS `DataViewModel` and `DataTabView` updated with new fields; DataTabView camera card gains MEDIA VOL / CLIP COUNT / SPACE REM row. (5) Codec variant parser corrected — BMPCC returns codec as colon-delimited string (e.g. "ProRes:Proxy", "BRAW:12:1"); split changed from space to colon; "H.265" (no colon) → base="H.265", variant=None. Note: BMPCC battery is permanently unavailable via REST API on all confirmed firmware versions.
