...
| `monitor` | Detector inference speed — mitigations applied, re-measure post-deploy | `detector.py` | Part 5: threads 2→3, ORT_SEQUENTIAL, cv2 letterbox. Part 6: 416×416 model re-export (~2x speedup expected). Baseline 0.53fps → target ≥1.4fps. Measure `detector_fps_actual` after restart. |
